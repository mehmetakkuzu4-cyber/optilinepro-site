  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/v1/public/release" && request.method === "GET") {
        return json(await currentRelease(env), 200, cors);
      }
      ...
      if (url.pathname === "/v1/admin/snapshot" && request.method === "GET") {
        const [licenses, release, requests, customers] = await Promise.all([
          listLicenses(env), currentRelease(env), listRequests(env), listCustomers(env)
        ]);
        return json({ licenses, release, requests, customers }, 200, cors);
      }
...
async function currentRelease(env) {
  ...
  if (!row) return { version: "1.1.1", date: "", setup_url: "", update_url: "", sha256: "", required: false, notes: "" };
  return {
    id: row.id,
    version: row.version,
  ...
  await env.DB.batch([
    ...
    ON CONFLICT(version) DO UPDATE SET
      ...
      .bind(releaseId, version, input.date || timestamp.slice(0, 10), input.setup_url || "", input.update_url || "",
        input.sha256 || "", input.required ? 1 : 0, input.notes || "", timestamp, timestamp),
